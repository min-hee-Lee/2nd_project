...
// http://localhost:8090/test/Main_host

@Controller
public class Main_hostController {

	@Autowired
	private Test_Service service;
	
	@GetMapping("/test/Main_host")
	public void execute() throws IOException {
		...
		Main_hostDTO main_hostDTO = new Main_hostDTO();
		
		main_hostDTO.setMain_code(num++);
		main_hostDTO.setMain_name(host.getName());
		main_hostDTO.setMain_address(host.getAddress());
		main_hostDTO.setMain_comment(host.getDescription());
		main_hostDTO.setLatitude(host.getLatitude());
		main_hostDTO.setLongitude(host.getLongitude());
		main_hostDTO.setMain_logo(host.getImage().getResourcePath());
		
		
		service.main_host(main_hostDTO);
